...
class DilatedCNNAD(AbstractModel):
    ...
    def add_normalizer(self, input_tensor, drop=False):
        tensor = batch_norm(input_tensor, center=True, scale=True, is_training=self.norm, renorm=True)
        if not drop:
            return tensor
    ...
    def get_res_unit(self, input_layer, fil_size, filter_sizes=(8, 32, 32),
        ...
        with tf.name_scope(scope_name):
            ...
            x = tf.contrib.layers.batch_norm(x, center=True, scale=True, is_training=self.norm, renorm=True)
    ...
    def build(self):
        ...
        with tf.variable_scope('predictor'):
            ...
            feature_tensors.append(latent)
            x = tf.nn.dropout(x, keep_prob=self.keep_prob)

            x = tf.slice(x, [0, 0, 0, 0], [tf.shape(x)[0], 12, 1, filters])
            features = tf.reshape(x, [-1, 12 * filters])

            x = slim.fully_connected(features, 256, activation_fn=tf.nn.selu,
                                     scope='fc1')
            x = add_norm(x)
            x = tf.nn.dropout(x, keep_prob=self.keep_prob)

            x = slim.fully_connected(x, 256, activation_fn=tf.nn.selu,
                                     scope='fc2')
            x = add_norm(x)
            x = tf.nn.dropout(x, keep_prob=self.keep_prob)
        ...
        with tf.variable_scope('classifier'):
            ...
            x_ad = slim.fully_connected(inverse_gradient_layer, 512, activation_fn=tf.nn.selu,
                                        scope='fc1_ad')
    ...
    def predict_on_batch(self, x, batch_size=4096):
        preds = []

        for i in range(0, x.shape[0], batch_size):
            x_batch = x[i: i + batch_size, :, :, :]
            _ = self.sess.run(self.predictions, {self.data: x_batch,
                                                 self.keep_prob: 1.0,
                                                 self.norm: 1.0})
